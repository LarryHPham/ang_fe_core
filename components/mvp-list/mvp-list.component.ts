...
export class MVPListComponent implements DoCheck, OnInit  {
  ...
  ngOnChanges(event) {
      ...
      console.log('event.tabs.currentValue - ',event.tabs.currentValue);
      console.log('tabData - ',this.tabData);
    //   if (this.tabs) {
    //       var matchingTab = this.tabs.filter( tab => tab.tabDisplayTitle == this.selectedTabTitle );
    //       console.log('matchingTab[0] - ',matchingTab[0]);
    //       console.log('matchingTab[0].listData - ',matchingTab[0].listData);
    //     //   this.updateCarousel(matchingTab[0]);
    //   }
